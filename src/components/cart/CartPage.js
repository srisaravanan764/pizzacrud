...
class CartPage extends Component {
  ...
  render () {
    ...
    var subtotal =  this.props.cart.map(c => c.subtotal);
    subtotal = subtotal.filter(Boolean);
    if(subtotal.length == 0){
      let toppings = JSON.parse(localStorage.getItem("toppings"));
      let toppingsId  = this.props.products.find(p => p._id === toppings.id)
        if(toppingsId){
          var subtotal = [parseInt(toppings.subtotal)];
        }else{
          var subtotal = [0];
        }
    }
